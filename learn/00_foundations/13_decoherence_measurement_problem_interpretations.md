...
Every noise channel in `docs/physics_module_design.md` is a decoherence model; every readout design decides which basis the environment (the amplifier chain) is allowed to learn. The Bell tests in `../../experiments/` are the empirical fact that no local classical story replaces the state.
