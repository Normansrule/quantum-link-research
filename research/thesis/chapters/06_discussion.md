...
The rate problem is separate from the memory problem and is worse. Diffraction over an astronomical unit leaves about 10⁻⁹ of the photons even for a 10 m receiver, so a bright source delivers pairs per hour, not per second. Multiplexing by a factor of about 2 × 10³ with a 1 m transmitter and a 10 m receiver, or 2 × 10⁶ with 30 cm optics (E9, computed in S08), or relays at intermediate points, which do not exist between Earth and Mars, are the only remedies. Relays at Sun–Earth L4/L5 solve a different problem, conjunction blackouts, and the model shows they keep a path open through every one.
